...
const Bill = () => {
  ...
  return <WithPageExist error={billInfoError} data={billInfo}><CasperBill billInfo={billInfo} course={course}
                                                                          store={billInfo} transaction={transaction}
                                                                          dispatch={dispatch}
...
const FakeBill = ({billInfo, transaction, dispatch, course}) => {
  ...
  return (
    <>
      ...
      {status !== 'Paid' && !transaction?.id && !payment?.transaction?.txHash && !payment?.cancelled ?
        (!sign ?
...
const CasperBill = ({billInfo, transaction, dispatch, router, store, course}) => {
  ...
  return (
    <>
      {billInfo?.cancelled &&
        <Col span={24} style={{padding: '20px 0 0 0', display: 'flex', justifyContent: 'center', background: 'white'}}>
          <div style={{color: 'red', fontSize: '24px'}}>{billInfo?.cancelled && 'Cancelled'}</div>
        </Col>
      }
      <Col span={24} style={{padding: '20px 0 0 20px', background: 'white'}}>
      ...
      {transactionExplorer === '' && ledgerWallets.length > 0 && status !== 'Paid' && !billInfo.cancelled &&
